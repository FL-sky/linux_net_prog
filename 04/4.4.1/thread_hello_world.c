...
int main()
{
    ...
    //tmp2 = pthread_join(thread1, &retval);//cannot join with thread1 again
    //printf("thread1 return value(retval) is %p\n", retval);
    //printf("thread1 return value(tmp2) is %d\n", tmp2);

    tmp2 = pthread_join(thread2, &retval);
    printf("thread2 return value(retval) is %p\n", retval);
    printf("thread2 return value(tmp2) is %d\n", tmp2);
    if (tmp2 != 0) {
